...
public class ArticleDao {
	...
	public Article insert(Connection conn, Article article) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement(
					"insert into article (article_no, writer_id, writer_name, title, regdate, moddate, read_cnt) values (baseq.nextval,?,?,?,?,?,0)");
	...
	public int update(Connection conn, int no, String title) throws SQLException{
		try (
			PreparedStatement pstmt = conn.prepareStatement("update article set title = ?, moddate = systimestamp where article_no = ?");){
	...
	public void delete(Connection conn, int no) throws SQLException {
		...
		try (
			PreparedStatement pstmt = conn.prepareStatement("delete article where article_no = ?");){
			pstmt.setInt(1, no);
			pstmt.executeQuery();
